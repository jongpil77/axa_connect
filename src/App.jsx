        ...
        <div className="p-5 space-y-6 pb-28 animate-fade-in bg-blue-50">
            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-blue-100">
                ...
                <div className="flex justify-between gap-2">
                    {[{ id: 'happy', icon: '😄', label: '최고예요' }, { id: 'soso', icon: '🙂', label: '괜찮아요' }, { id: 'sad', icon: '😥', label: '지쳤어요' }].map((item) => (
                        <button key={item.id} onClick={() => handleMoodCheck(item.id)} disabled={!!mood} className={`flex-1 flex flex-col items-center p-3 rounded-2xl transition-all ${mood === item.id ? 'ring-2 ring-blue-500 scale-105 bg-white shadow-md' : 'hover:bg-slate-50'} ${mood && mood !== item.id ? 'opacity-50' : ''}`}>
            ...
            {(weeklyBirthdays.current.length > 0 || weeklyBirthdays.next.length > 0) && (
                <div className="bg-gradient-to-br from-pink-50 to-white p-5 rounded-[2rem] shadow-sm border border-pink-100">
                    <h3 className="text-sm font-bold text-pink-600 flex items-center gap-2 mb-3"><Gift className="w-4 h-4"/> 생일 축하해주세요!</h3>
                    <div className="space-y-3">
                        {weeklyBirthdays.current.length > 0 && (<div><p className="text-xs font-bold text-slate-400 mb-1">이번 주</p><div className="flex flex-wrap gap-2">{weeklyBirthdays.current.map((b, i) => (<span key={i} className="px-3 py-1 bg-white rounded-full text-xs font-bold text-slate-700 shadow-sm border border-pink-100">🎉 {b.name} ({b.date})</span>))}</div></div>)}
                        {weeklyBirthdays.next.length > 0 && (<div><p className="text-xs font-bold text-slate-400 mb-1">다음 주</p><div className="flex flex-wrap gap-2">{weeklyBirthdays.next.map((b, i) => (<span key={i} className="px-3 py-1 bg-white/50 rounded-full text-xs text-slate-500 border border-slate-100">{b.name} ({b.date})</span>))}</div></div>)}
                    </div>
                </div>
            )}
        </div>
        ...
        <div className="p-5 space-y-4 pb-28 animate-fade-in bg-blue-50">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-blue-100 flex justify-between items-center">
                <h2 className="text-lg font-black text-slate-800 flex items-center gap-2"><Bell className="w-5 h-5 text-red-500"/> 공지사항</h2>
                 {currentUser?.role === 'admin' && (<button onClick={onWriteClick} className="text-xs bg-slate-800 text-white px-3 py-1.5 rounded-lg font-bold">글쓰기</button>)}
            </div>
            {newsFeeds.length > 0 ? (
                <div className="space-y-3">
                    {newsFeeds.map(feed => (
                         <div key={feed.id} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                            <div className="flex justify-between items-start mb-2"><h3 className="text-sm font-bold text-slate-800">{feed.title}</h3><span className="text-[10px] text-slate-400">{feed.formattedTime}</span></div>
                            <p className="text-xs text-slate-600 leading-relaxed whitespace-pre-wrap">{feed.content}</p>
                             {feed.image_url && (<div className="mt-3 rounded-xl overflow-hidden"><img src={feed.image_url} alt="Notice" className="w-full h-auto" /></div>)}
                         </div>
                    ))}
                </div>
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
             <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl relative text-center overflow-hidden">
                ...
                <div className="mb-4 flex justify-center"><div className="w-20 h-20 bg-pink-50 rounded-full flex items-center justify-center text-4xl shadow-inner">🎂</div></div>
                <h3 className="text-xl font-black text-slate-800 mb-2">생일 축하합니다!</h3>
                <p className="text-sm text-slate-600 mb-6">{userName}님,<br/>오늘 하루 가장 행복한 사람이 되세요! 🎉</p>
                <div className="bg-pink-50 p-3 rounded-xl mb-6"><p className="text-xs font-bold text-pink-600">🎁 생일 축하 선물</p><p className="text-lg font-black text-pink-500">+1,000 P</p></div>
                <button onClick={handleBirthdayGrant} className="w-full bg-gradient-to-r from-pink-500 to-red-500 text-white p-3 rounded-xl font-bold hover:shadow-lg transition-all active:scale-95">선물 받기</button>
    ...
    return (
        <div className="fixed bottom-24 left-1/2 transform -translate-x-1/2 z-50 animate-fade-in-up w-[90%] max-w-sm">
            <div className="bg-slate-800/90 backdrop-blur-sm text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border border-slate-700">
                <span className="text-3xl">{emoji}</span><span className="text-sm font-bold leading-relaxed">{message}</span>
            </div>
        </div>
    );
};
      ...
      <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-8 border border-blue-100 animate-fade-in relative overflow-hidden">
        ...
        {isSignupMode ? (
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div><label className="block text-xs font-bold text-slate-500 mb-1 ml-1">생년월일</label><div className="flex gap-2"><input name="birthdate" type="date" value={birthdate} onChange={(e) => setBirthdate(e.target.value)} className="flex-1 p-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none text-sm text-slate-600 focus:border-blue-500 transition-colors" required /><div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-3 shrink-0"><label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="calendarType" value="solar" checked={calendarType === 'solar'} onChange={() => setCalendarType('solar')} className="w-3 h-3 text-blue-600" /><span className="text-xs text-slate-600">양력</span></label><label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="calendarType" value="lunar" checked={calendarType === 'lunar'} onChange={() => setCalendarType('lunar')} className="w-3 h-3 text-blue-600" /><span className="text-xs text-slate-600">음력</span></label></div></div></div>
            ...
            <div className="space-y-2 bg-slate-50 p-3 rounded-2xl border border-slate-100"><div className="grid grid-cols-2 gap-2"><select name="dept" className="w-full p-2 bg-white border border-slate-200 rounded-xl outline-none text-xs text-slate-700" onChange={(e) => setSelectedDept(e.target.value)} required><option value="">본부/부문</option>{Object.keys(ORGANIZATION).map(dept => <option key={dept} value={dept}>{dept}</option>)}</select><select name="team" className="w-full p-2 bg-white border border-slate-200 rounded-xl outline-none text-xs text-slate-700" disabled={!selectedDept} required><option value="">팀/센터</option>{selectedDept && ORGANIZATION[selectedDept].map(team => <option key={team} value={team}>{team}</option>)}</select></div></div>
            <div><p className="text-[10px] text-slate-400 mb-1 ml-1">⚠️ 관리자 권한 부여 시에만 인증 코드 입력 (일반 사용자는 공란)</p><input name="code" type="password" placeholder="인증 코드 (선택)" className="w-full p-3.5 bg-white border-2 border-slate-100 rounded-2xl outline-none text-sm text-slate-800 placeholder-slate-300 focus:border-blue-500" /></div>
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md p-4 sticky top-0 z-30 border-b border-slate-100 shadow-sm">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2"><img src={AXA_LOGO_URL} alt="AXA Logo" className="w-8 h-auto" /><h1 className="text-lg font-black text-slate-800 tracking-tight">AXA Connect</h1></div>
        <div className="flex items-center gap-2 relative">
          <div className="bg-white text-slate-600 px-3 py-1.5 rounded-2xl text-xs font-bold flex items-center gap-2 border border-slate-100 shadow-sm"><Coins className="w-6 h-6 text-yellow-400 fill-yellow-400" /><div className="flex flex-col items-start leading-none"><span className="text-[9px] text-slate-400 font-normal mb-0.5">내 포인트</span><span className="text-sm font-black text-slate-700">{currentUser.points?.toLocaleString() || 0} P</span></div></div>
          <button onClick={onOpenUserInfo} className="w-9 h-9 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center text-white font-bold text-xs shadow-md hover:ring-2 ring-blue-200 transition-all">{displayName}</button>
          ...
          {showSettings && (
             <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden z-50 animate-fade-in">
                ...
                {currentUser.role === 'admin' && (<><button onClick={() => { setShowSettings(false); onOpenAdminGrant(); }} className="flex items-center gap-2 w-full p-3 text-xs text-blue-600 font-bold hover:bg-blue-50 border-b border-slate-50 transition-colors"><Gift className="w-3.5 h-3.5 text-blue-500"/> 포인트 지급 (관리자)</button><button onClick={() => { setShowSettings(false); onOpenRedemptionList(); }} className="flex items-center gap-2 w-full p-3 text-xs text-purple-600 font-bold hover:bg-purple-50 border-b border-slate-50 transition-colors"><ClipboardList className="w-3.5 h-3.5 text-purple-500"/> 포인트 차감 신청 관리 (관리자)</button></>)}
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white w-full max-w-xs rounded-2xl p-6 shadow-2xl relative">
                ...
                <div className="space-y-3"><input type="password" placeholder="새 비밀번호 (6자리 이상 숫자)" className="w-full p-3 bg-slate-50 rounded-xl text-sm border border-slate-200 outline-none" value={password} onChange={(e) => setPassword(e.target.value)} />{!isValid && password.length > 0 && <p className="text-[10px] text-red-500">6자리 이상 숫자로 입력해주세요.</p>}<button onClick={() => onSave(password)} disabled={!isValid} className="w-full bg-blue-600 text-white p-3 rounded-xl font-bold hover:bg-blue-700 disabled:bg-slate-300 transition-colors">비밀번호 변경</button></div>
            </div>
        </div>
    );
};
...
const UserInfoModal = ({ currentUser, pointHistory, setShowUserInfoModal, handleRedeemPoints }) => (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white w-full max-w-md rounded-[2rem] p-0 shadow-2xl max-h-[90vh] overflow-y-auto relative">
        <div className="bg-gradient-to-r from-blue-600 to-blue-500 p-6 rounded-t-[2rem] flex justify-between items-center sticky top-0 z-10"><div className="flex flex-col text-white"><h3 className="text-lg font-bold flex items-center gap-2"><User className="w-5 h-5"/> {currentUser.name}</h3><p className="text-xs opacity-90 ml-7 mt-0.5 flex items-center gap-1 font-medium"><Building2 className="w-3 h-3"/> {currentUser.dept} / {currentUser.team}</p></div><button onClick={() => setShowUserInfoModal(false)} className="p-2 bg-white/20 hover:bg-white/30 rounded-full text-white transition-colors"><X className="w-5 h-5" /></button></div>
        <div className="p-6 space-y-5">
            {currentUser.points >= 10000 ? (<div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 text-center"><p className="text-sm text-blue-800 font-bold mb-2">🎉 보유 포인트가 10,000P 이상입니다!</p><button onClick={handleRedeemPoints} className="w-full bg-blue-600 text-white py-3 rounded-xl text-sm font-bold hover:bg-blue-700 flex items-center justify-center gap-2 transition-colors shadow-md"><Wallet className="w-4 h-4" /> 10,000P 상품권 교환 신청</button></div>) : (<div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center"><p className="text-xs text-slate-500">10,000P 부터 상품권 교환 신청이 가능해요 🎁</p><div className="mt-2 w-full bg-slate-200 h-2 rounded-full overflow-hidden"><div className="bg-blue-400 h-full transition-all duration-500" style={{ width: `${Math.min((currentUser.points / 10000) * 100, 100)}%` }}></div></div><p className="text-[10px] text-slate-400 mt-1 text-right">{Math.floor((currentUser.points / 10000) * 100)}% 달성</p></div>)}
            <div>
...
const FeedTab = ({ feeds = [], activeFeedFilter, setActiveFeedFilter, onWriteClick, currentUser, handleDeletePost, handleLikePost, handleAddComment, handleDeleteComment }) => {
  ...
  return (
    <div className="p-5 space-y-5 pb-28 animate-fade-in bg-blue-50">
      <div className="bg-white p-2 rounded-2xl shadow-sm border border-blue-100 flex items-center gap-2"><Search className="w-4 h-4 text-slate-400 ml-2" /><input type="text" placeholder="검색어를 입력하세요 (제목, 내용, 작성자)" className="flex-1 bg-transparent text-xs p-2 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">{[{ id: 'all', label: '전체' }, { id: 'praise', label: '칭찬해요' }, { id: 'knowhow', label: '업무 꿀팁' }, { id: 'matjib', label: '맛집 소개' }].map(tab => (<button key={tab.id} onClick={() => setActiveFeedFilter(tab.id)} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${activeFeedFilter === tab.id ? 'bg-slate-800 text-white border-slate-800 shadow-md' : 'bg-white text-slate-500 border-slate-200'}`}>{tab.label}</button>))}</div>
      <div className="flex items-center justify-between mb-1"><div className="flex items-center gap-2 cursor-pointer" onClick={onWriteClick}><div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-4 py-2.5 rounded-full shadow-lg hover:shadow-xl transition-all flex items-center gap-2 active:scale-95 border border-blue-400"><div className="bg-white/20 px-1.5 py-0.5 rounded text-[10px] font-bold flex items-center"><Coins className="w-2.5 h-2.5 text-yellow-300 fill-yellow-300 mr-0.5"/>100P</div><Pencil className="w-3.5 h-3.5" /><span className="text-xs font-bold">게시글 작성</span></div></div><p className="text-[9px] text-blue-500 font-bold bg-blue-100 px-2 py-1 rounded-lg">게시물 1개 작성시 +100P (일 최대 300P 가능)</p></div>
      {filteredFeeds.map(feed => {
        ...
        return (
          <div key={feed.id} className="bg-white rounded-3xl p-5 shadow-sm border border-blue-100 relative group transition-all hover:shadow-md">
            <div className="flex items-center gap-3 mb-3"><div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-white text-sm bg-gradient-to-br from-blue-500 to-blue-400 shadow-sm`}>{formatInitial(feed.author)}</div><div><p className="text-sm font-bold text-slate-800 flex items-center gap-1">{feed.author} {feed.profiles?.role === 'admin' && <span className="bg-red-50 text-red-500 text-[9px] px-1.5 py-0.5 rounded-md border border-red-100">관리자</span>}</p><p className="text-[10px] text-slate-400">{feed.formattedTime} • {feed.team}</p></div></div>
            <div className="mb-4"><div className="flex flex-wrap gap-1 mb-2"><span className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-bold border ${feed.type === 'praise' ? 'bg-green-50 text-green-600 border-green-100' : feed.type === 'news' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>{feed.type === 'praise' ? '칭찬해요' : feed.type === 'news' ? '📢 공지사항' : feed.type === 'matjib' ? '맛집 소개' : '업무 꿀팁'}</span>{feed.region_main && <span className="inline-block px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-50 text-slate-500 border border-slate-200"><MapPin className="w-2.5 h-2.5 inline mr-0.5"/>{feed.region_main} {feed.region_sub}</span>}</div>{feed.type === 'praise' && feed.target_name && <p className="text-xs font-bold text-green-600 mb-1">To. {feed.target_name}</p>}{feed.title && (<h3 className="text-base font-bold text-slate-800 mb-1.5">{feed.title}{isToday(feed.created_at) && <span className="ml-1 px-1 py-0.5 bg-red-500 text-white text-[8px] font-bold rounded-sm inline-block">NEW</span>}</h3>)}<p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap">{feed.content}</p>{feed.link_url && (<a href={feed.link_url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs text-blue-500 bg-blue-50 px-2 py-1.5 rounded-lg hover:underline w-full truncate"><LinkIcon className="w-3 h-3 flex-shrink-0" /> <span className="truncate">{feed.link_url}</span></a>)}</div>
            {feed.image_url && (<div className="mb-4 rounded-2xl overflow-hidden border border-slate-100 shadow-sm"><img src={feed.image_url} alt="Content" className="w-full h-full object-cover" /></div>)}
            <div className="flex items-center gap-4 border-t border-slate-50 pt-3"><button onClick={() => handleLikePost(feed.id, feed.likes, feed.isLiked)} className={`flex items-center gap-1 text-xs font-bold transition-colors ${feed.isLiked ? 'text-red-500' : 'text-slate-400 hover:text-slate-600'}`}><Heart className={`w-4 h-4 ${feed.isLiked ? 'fill-red-500' : ''}`} /> {feed.likes?.length || 0}</button><div className="flex items-center gap-1 text-xs font-bold text-slate-400"><MessageCircle className="w-4 h-4" /> {comments.length}</div><div className="ml-auto text-[10px] text-slate-300">{feed.formattedTime}</div>{(currentUser?.id === feed.author_id || currentUser?.role === 'admin') && (<button onClick={() => handleDeletePost(feed.id)} className="text-[10px] text-slate-400 hover:text-red-500 transition-colors flex items-center gap-1 px-2 py-1">삭제</button>)}</div>
            ...
            <form onSubmit={(e) => handleAddComment(e, feed.id, null)} className="flex gap-2 mt-3"><input name="commentContent" type="text" placeholder="댓글을 남겨주세요..." className="flex-1 px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-xs outline-none focus:border-blue-400 focus:bg-white transition-colors" required /><button type="submit" className="bg-white border border-slate-200 text-slate-500 p-2 rounded-xl hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-colors"><Send className="w-3.5 h-3.5"/></button></form>
          </div>
        );
      })}
    </div>
...
const RankingTab = ({ feeds, profiles, allPointHistory }) => {
    ...
    const RankItem = ({ rank, name, value, unit, team, color }) => (<div className="flex items-center p-3 bg-white border border-slate-100 rounded-2xl shadow-sm"><div className={`text-xl font-black mr-4 w-8 text-center ${color}`}>{rank}</div><div className="flex-1"><p className="text-sm font-bold text-slate-800">{name || 'Unknown'}</p><p className="text-[10px] text-slate-400">{team}</p></div><div className="text-base font-black text-slate-700 ml-4">{value}<span className="text-[10px] text-slate-400 ml-0.5 font-normal">{unit}</span></div></div>);
    return (
        <div className="p-5 space-y-8 pb-28 animate-fade-in bg-blue-50">
            <div className="bg-white p-5 rounded-[2rem] shadow-sm border border-blue-100 text-center relative"><div className="flex justify-between items-center mb-4 px-2"><button onClick={handlePrevMonth} className="p-1 hover:bg-slate-100 rounded-full"><ChevronLeft className="w-5 h-5 text-slate-400" /></button><h2 className="text-lg font-black text-slate-800">{selectedDate.getFullYear()}년 {selectedDate.getMonth() + 1}월 랭킹</h2><button onClick={handleNextMonth} className="p-1 hover:bg-slate-100 rounded-full disabled:opacity-30" disabled={selectedDate >= new Date(new Date().setDate(1))}><ChevronRight className="w-5 h-5 text-slate-400" /></button></div><p className="text-xs text-slate-400">매월 1일 ~ 말일 기준 집계</p></div>
...
export default function App() {
  ...
  const [debugStatus, setDebugStatus] = useState("앱 초기화 중..."); // 디버그용 상태
  ...
  // [강제 진입 로직] 타임아웃 발생 시 임시 유저 생성
  const forceEntry = useCallback((userId, email) => {
      console.warn("⚠️ 강제 진입 모드 발동: 임시 프로필 생성");
      setDebugStatus("데이터 응답 없음. 임시 프로필로 진입합니다.");
      const fallbackUser = { 
          id: userId, 
          name: '임시 회원', 
          email: email, 
  ...
  const fetchUserData = useCallback(async (userId, userEmail) => {
    ...
    try {
        ...
        if (error) {
            ...
            // 데이터가 null인 경우 (DB에는 접속했으나 데이터가 없음)
            forceEntry(userId, userEmail);
        }
    } catch (err) { 
  ...
  // [수정: fetchFeeds 의존성 제거] currentUser를 의존성 배열에서 제거하여 무한 루프 방지
  const fetchFeeds = useCallback(async () => {
    ...
    try {
        ...
        // comments 테이블에서 전체 데이터를 가져오는 대신, 필요한 데이터만 가져오는 것이 좋으나 현재 구조상 유지
        // 단, comments 변수가 null일 경우 빈 배열로 처리하도록 안전장치 추가
        const { data: commentsData } = await supabase.from('comments').select(`*, profiles:author_id (name, role)`).order('created_at', { ascending: true });
        
        const comments = commentsData || []; // null check
        ...
        if (posts) {
            ...
            const formatted = posts.map(post => {
                ...
                const formattedTime = createdDate.toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/\. /g, '.').replace(/\./g, '/').slice(0, -1) + ' ' + createdDate.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false }).replace(' ', ''); 
                ...
                return { 
                    ...
                    isLiked: false, // 초기값 false (useEffect에서 업데이트)
        ...
        try {
            ...
            if (mounted) {
                ...
                if (session) {
                    ...
                    // [핵심] 안전장치: 2초 뒤에도 currentUser가 없으면 강제 진입
                    timerId = setTimeout(() => {
                        if (mounted) {
                            setDebugStatus("응답 지연. 강제 진입 시도...");
                            const fallbackUser = { id: session.user.id, name: '임시 회원', email: session.user.email, role: 'member', points: 0, dept: '접속지연', team: '-' };
                            setCurrentUser(prev => prev ? prev : fallbackUser);
                        }
                    }, 2000);
                } else {
  ...
  // [수정됨] 좋아요 상태 별도 업데이트 (댓글 수 깜빡임 방지)
  useEffect(() => {
      if (currentUser && feeds.length > 0) {
          setFeeds(prevFeeds => prevFeeds.map(f => ({ ...f, isLiked: f.likes.includes(currentUser.id) })));
      }
  }, [currentUser?.id]); 
  ...
  const handleAddComment = async (e, postId, parentId = null) => {
      ...
      try { await supabase.from('comments').insert({ post_id: postId, author_id: currentUser.id, content: content, parent_id: parentId }); e.target.reset(); } catch (err) { console.error('댓글 작성 실패: ', err.message); }
  };
  
  const handleDeleteComment = async (commentId) => {
      if (!window.confirm('댓글을 삭제하시겠습니까?')) return;
      try { await supabase.from('comments').delete().eq('id', commentId); } catch (err) { console.error('삭제 실패: ', err.message); }
  };
  ...
  const handleLogin = async (e) => {
    ...
    try {
        ...
        if (error) {
            if (userCheck === null) { alert('가입되지 않은 이메일 계정입니다.'); } else { alert('비밀번호가 일치하지 않습니다.'); }
        }
  ...
  const handleSignup = async (e) => {
    ...
    try {
        const initialData = { name: name.value, dept: dept.value, team: team.value, role: role, points: INITIAL_POINTS, birthdate: birthdate.value, calendar_type: calendarType.value, email: email.value };
  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
        ...
        const { error: postError } = await supabase.from('posts').insert({ content: content, type: category, author_id: currentUser.id, image_url: publicImageUrl, target_name: targetName, title: title, region_main: regionMain, region_sub: regionSub, link_url: linkUrl, likes: [] });
  ...
  const handleMoodCheck = async (selectedMood) => {
    ...
    if (selectedMood === 'happy') { message = "오늘 기분 최고예요! 뭐든 할 준비 완료! 😄"; emoji = "✨"; } else if (selectedMood === 'soso') { message = "괜찮아요! 오늘도 잘 해낼 거예요! 💪"; emoji = "🍀"; } else if (selectedMood === 'sad') { message = "조금 지쳤지만 버틸 수 있어요.. 🐌"; emoji = "☕"; }
  ...
  const handleLogout = async () => {
    if (!supabase) return; 
    try { await supabase.auth.signOut(); setCurrentUser(null); setSession(null); setMood(null); setPointHistory([]); } catch (err) { console.error('로그아웃 실패: ', err.message); }
  };

  const handleChangePasswordClick = async () => {
    if (!currentUser || !supabase) return; 
    if (!window.confirm('비밀번호를 초기화(15661566) 하시겠습니까?')) return;
    try { const { error } = await supabase.auth.updateUser({ password: '15661566' }); if (error) throw error; } catch (err) { console.error('변경 실패: ', err.message); }
  };

  const handleChangeDept = async (newDept, newTeam) => {
    if (!currentUser || !supabase) return;
    try { await supabase.from('profiles').update({ dept: newDept, team: newTeam }).eq('id', currentUser.id); fetchUserData(currentUser.id, currentUser.email); setShowChangeDeptModal(false); alert('소속이 변경되었습니다.'); } catch(err) { console.error(err); }
  };

  const handleChangePassword = async (newPassword) => {
    if (!currentUser || !supabase) return;
    try { const { error } = await supabase.auth.updateUser({ password: newPassword }); if (error) throw error; setShowChangePwdModal(false); alert('비밀번호가 변경되었습니다. 다시 로그인해주세요.'); handleLogout(); } catch(err) { console.error(err); }
  };
  ...
  const handleAdminGrantPoints = async (targetUserId, amount) => {
      ...
      try {
          ...
          await supabase.from('point_history').insert({ user_id: targetUserId, reason: '관리자 특별 지급', amount: parseInt(amount), type: 'earn' });
            ...
            <>
              {!currentUser ? (
                  <div className="flex-1 flex flex-col items-center justify-center h-full gap-2">
                      ...
                      <p className="text-sm text-slate-500 font-bold">사용자 정보를 불러오는 중...</p>
                      <p className="text-xs text-slate-400">{debugStatus}</p>
                  </div>
              ) : (
                  <>
                      ...
                      <main className="flex-1 overflow-y-auto scrollbar-hide">
                        {activeTab === 'home' && (<HomeTab mood={mood} handleMoodCheck={handleMoodCheck} feeds={feeds} weeklyBirthdays={weeklyBirthdays} onWriteClick={() => setShowWriteModal(true)} onNavigateToNews={() => setActiveTab('news')} onNavigateToFeed={handleNavigateToFeedWithFilter} />)}
                        {activeTab === 'feed' && (<FeedTab feeds={feeds} activeFeedFilter={activeFeedFilter} setActiveFeedFilter={setActiveFeedFilter} onWriteClick={() => setShowWriteModal(true)} currentUser={currentUser} handleDeletePost={handleDeletePost} handleLikePost={handleLikePost} handleAddComment={handleAddComment} handleDeleteComment={handleDeleteComment} />)}
