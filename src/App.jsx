...
             currentBirthdays.push({ name: p.name, date: `${m}/${d}`, typeLabel: p.calendar_type === 'lunar' ? '(-)' : '(+)' });
        } 
        else if (normalizedBirthDate >= normalizeDate(endOfCurrentWeek) && normalizedBirthDate < normalizeDate(endOfNextWeek)) {
             nextBirthdays.push({ name: p.name, date: `${m}/${d}`, typeLabel: p.calendar_type === 'lunar' ? '(-)' : '(+)' });
        }
    });
    ...
    try {
        ...
        if (posts) {
            const buildCommentTree = (postComments) => {
                const commentMap = {};
                const rootComments = [];
                // 댓글을 ID로 매핑하고 replies 배열 초기화
                postComments.forEach(c => { commentMap[c.id] = { ...c, replies: [] }; });
                // 모든 댓글을 루트 댓글로 추가 (실제 대댓글 구조는 없으므로 간단히 처리)
            ...
            const formatted = posts.map(post => {
                ...
                // isLiked 계산 시 currentUser가 있을 때만 처리
                const likesArray = post.likes ? (Array.isArray(post.likes) ? post.likes : JSON.parse(post.likes)) : [];
                ...
                return {
                    ...
                    likes: likesArray, 
                    isLiked: currentUser ? likesArray.includes(currentUser.id) : false,
                    comments: buildCommentTree(postComments), 
                    totalComments: postComments.length 
                };
            });
            
            // 기존 feeds와 비교하여 실제로 변경된 경우에만 업데이트 (깜빡임 최소화)
            setFeeds(prevFeeds => {
                // 단순한 길이 비교나 JSON.stringify는 비효율적일 수 있으나,
                // 여기서는 객체 참조가 달라져도 내용이 같으면 업데이트하지 않도록
                // 깊은 비교 대신, 배열 길이가 같고, 각 항목의 ID/댓글 수가 같으면 업데이트 방지
                if (prevFeeds.length !== formatted.length) return formatted;

                for (let i = 0; i < formatted.length; i++) {
                    const prev = prevFeeds.find(f => f.id === formatted[i].id);
                    if (!prev || prev.totalComments !== formatted[i].totalComments || prev.likes.length !== formatted[i].likes.length) {
                        return formatted;
                    }
                }
                return prevFeeds;
...
  useEffect(() => {
    ...
    // 테이블 변경 시 debouncedFetchFeeds를 통해 데이터 갱신
    const channel = supabase.channel('public:comments_posts')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'comments' }, debouncedFetchFeeds)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'posts' }, debouncedFetchFeeds)
        .subscribe();
    ...
    try {
        ...
        return () => {
            ...
            clearTimeout(debounceTimer); // 클린업 시 타이머 제거
        };
