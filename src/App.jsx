...
import { User, Heart, MessageCircle, Gift, Bell, Sparkles, Smile, Frown, Meh, Megaphone, X, Send, Settings, ChevronRight, LogOut, Image as ImageIcon, Coins, Pencil, Trash2, Loader2, Lock, Clock, Award, Wallet, Building2, CornerDownRight, Link as LinkIcon, MapPin, Search, Key, Edit3, ClipboardList, CheckSquare, ChevronLeft, Coffee } from 'lucide-react';
...
// Header: 일반 사용자 포인트 선물하기 추가, 내 포인트 명칭 변경
const Header = ({ currentUser, onOpenUserInfo, handleLogout, onOpenChangeDept, onOpenChangePwd, onOpenAdminGrant, onOpenUserGift, onOpenRedemptionList }) => {
        ...
        <div className="flex items-center gap-2 relative">
          <div className="bg-white text-slate-600 px-3 py-1.5 rounded-2xl text-xs font-bold flex items-center gap-2 border border-slate-100 shadow-sm">
             ...
             <div className="flex flex-col items-start leading-none">
                 <span className="text-[9px] text-slate-400 font-normal mb-0.5">My Care Point</span>
          ...
          {showSettings && (
             <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden z-50 animate-fade-in">
                ...
                <button onClick={() => { setShowSettings(false); onOpenChangePwd(); }} className="flex items-center gap-2 w-full p-3 text-xs text-slate-600 hover:bg-slate-50 border-b border-slate-50 transition-colors">
                   <Key className="w-3.5 h-3.5 text-blue-400"/> 비밀번호 초기화
                </button>
                
                {/* 일반 사용자도 포인트 선물 가능 (관리자 제외 또는 포함 정책에 따라 조정 가능) */}
                {currentUser?.role !== 'admin' && (
                    <button onClick={() => { setShowSettings(false); onOpenUserGift(); }} className="flex items-center gap-2 w-full p-3 text-xs text-pink-600 font-bold hover:bg-pink-50 border-b border-slate-50 transition-colors">
                        <Gift className="w-3.5 h-3.5 text-pink-500"/> 포인트 선물하기
                    </button>
                )}
...
const ChangePasswordModal = ({ onClose, onSave }) => {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white w-full max-w-xs rounded-2xl p-6 shadow-2xl relative">
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-400"><X className="w-5 h-5"/></button>
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Key className="w-5 h-5"/> 비밀번호 초기화</h3>
                <p className="text-sm text-slate-600 mb-6 text-center">비밀번호를 <strong>15661566</strong>으로 초기화하시겠습니까?</p>
                <button onClick={() => onSave('15661566')} className="w-full bg-blue-600 text-white p-3 rounded-xl font-bold hover:bg-blue-700 transition-colors">초기화 진행</button>
            </div>
        </div>
    );
};

// 일반 사용자 포인트 선물 모달 (New)
const UserGiftModal = ({ onClose, onGift, profiles, currentUser }) => {
    const [dept, setDept] = useState('');
    const [targetUser, setTargetUser] = useState('');
    const [amount, setAmount] = useState('');

    // 본인 제외하고 목록 표시
    const filteredUsers = profiles.filter(p => p.dept === dept && p.id !== currentUser.id);

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl relative">
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-400"><X className="w-5 h-5"/></button>
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-pink-600"><Gift className="w-5 h-5"/> 포인트 선물하기</h3>
                <p className="text-xs text-slate-500 mb-4">나의 보유 포인트: <span className="font-bold text-slate-800">{currentUser.points?.toLocaleString()} P</span></p>
                <div className="space-y-3">
                    <select className="w-full p-3 bg-slate-50 rounded-xl text-sm border border-slate-200 outline-none" onChange={(e) => { setDept(e.target.value); setTargetUser(''); }}>
                        <option value="">받는 분 소속 선택</option>
                        {Object.keys(ORGANIZATION).map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                    <select className="w-full p-3 bg-slate-50 rounded-xl text-sm border border-slate-200 outline-none" disabled={!dept} onChange={(e) => setTargetUser(e.target.value)}>
                        <option value="">받는 분 이름 선택</option>
                        {filteredUsers.map(u => <option key={u.id} value={u.id}>{u.name} ({u.team})</option>)}
                    </select>
                    <input 
                        type="number" 
                        placeholder="선물할 포인트 (숫자만 입력)" 
                        className="w-full p-3 bg-slate-50 rounded-xl text-sm border border-slate-200 outline-none font-bold"
                        value={amount}
                        onChange={(e) => setAmount(e.target.value)}
                    />
                    <button onClick={() => onGift(targetUser, amount)} disabled={!targetUser || !amount} className="w-full bg-gradient-to-r from-pink-500 to-pink-600 text-white p-3 rounded-xl font-bold hover:shadow-lg disabled:opacity-50 transition-all">
                        선물 보내기
                    </button>
                </div>
            </div>
...
// ... UserInfoModal, BirthdayPopup, BirthdayNotifier, Comment components remain the same ...
...
const HomeTab = ({ mood, handleMoodCheck, feeds, onWriteClick, onNavigateToNews, onNavigateToFeed, weeklyBirthdays }) => {
    ...
    const noticeFeed = noticeFeedsAll.slice(0, 1); // 공지사항 최신 1개만

    const praiseFeeds = feeds.filter(f => f.type === 'praise').slice(0, 3); 
    const ourNewsFeeds = feeds.filter(f => f.type === 'our_news').slice(0, 3); // 우리들 소식
    const tipsFeeds = feeds.filter(f => f.type === 'knowhow').slice(0, 3); // 꿀팁
    const matjibFeeds = feeds.filter(f => f.type === 'matjib').slice(0, 3); // 맛집
    ...
    // 섹션 렌더링 헬퍼
    const renderSection = (title, icon, type, data, colorClass, bgClass) => (
        <div className="bg-white p-4 rounded-3xl shadow-sm border border-blue-100 cursor-pointer mb-4" onClick={() => handleSectionClick(type)}>
            <h3 className={`text-sm font-bold ${colorClass} mb-3 flex items-center gap-1.5 pointer-events-none`}>{icon} {title}</h3>
            <div className="space-y-2 pointer-events-none">
                {data.length > 0 ? data.map(feed => (
                <div key={feed.id} className={`p-3 ${bgClass} rounded-2xl border border-slate-100 transition-colors`}>
                    {feed.type === 'praise' && <p className="text-[10px] font-bold text-slate-500 mb-1">To. {feed.target_name || '동료'}</p>}
                ...
                )) : <div className="text-center text-[10px] text-slate-400 py-2">게시글이 없습니다.</div>}
            </div>
        </div>
    );
    ...
    return (
      <div className="p-5 space-y-6 pb-32 animate-fade-in relative bg-blue-50 min-h-full">
            ...
            <div className="col-span-1">
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-blue-100 h-full flex flex-col">
                  ...
                  <div className="flex gap-2 h-full mt-2">
                    <button onClick={() => handleMoodCheck('happy')} className={`flex-1 rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all active:scale-90 duration-200 border ${getMoodButtonStyle('happy')}`} disabled={!!mood}>
                      <Smile className="w-5 h-5"/><span className="text-[9px] font-bold">좋음</span>
                    </button>
                    <button onClick={() => handleMoodCheck('soso')} className={`flex-1 rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all active:scale-90 duration-200 border ${getMoodButtonStyle('soso')}`} disabled={!!mood}>
                      <Meh className="w-5 h-5"/><span className="text-[9px] font-bold">보통</span>
                    </button>
                    <button onClick={() => handleMoodCheck('sad')} className={`flex-1 rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all active:scale-90 duration-200 border ${getMoodButtonStyle('sad')}`} disabled={!!mood}>
                      <Frown className="w-5 h-5"/><span className="text-[9px] font-bold">피곤</span>
                    </button>
                  </div>
        ...
        <div>
           ...
           <div className="space-y-2">
             {noticeFeed.length > 0 ? noticeFeed.map(feed => ( 
        ...
        <div className="space-y-4 min-h-[300px]">
            {/* 1. 칭찬합시다 */}
            {renderSection('칭찬합시다', <Heart className="w-4 h-4 fill-green-500 text-green-500"/>, 'praise', praiseFeeds, 'text-green-600', 'bg-green-50/30')}
            
            {/* 2. 우리들 소식 */}
            {renderSection('우리들 소식', <Smile className="w-4 h-4 text-pink-500"/>, 'our_news', ourNewsFeeds, 'text-pink-600', 'bg-pink-50/30')}
            
            {/* 3. 꿀팁 & 정보 */}
            {renderSection('꿀팁 & 정보', <Sparkles className="w-4 h-4 fill-blue-500 text-blue-500"/>, 'knowhow', tipsFeeds, 'text-blue-600', 'bg-blue-50/50')}
            
            {/* 4. 맛집 소개 */}
            {renderSection('맛집 소개', <Coffee className="w-4 h-4 text-orange-500"/>, 'matjib', matjibFeeds, 'text-orange-600', 'bg-orange-50/50')}
        </div>
      </div>
    );
};

// ... NoticeBoard, Comment, FeedTab, RankingTab ...
...
const WriteModal = ({ setShowWriteModal, handlePostSubmit, currentUser, activeTab }) => {
  ...
  const categories = useMemo(() => [
    ...
    {id: 'our_news', label: '우리들 소식', show: activeTab !== 'news'}, // NEW
    {id: 'matjib', label: '맛집소개', show: activeTab !== 'news'},
    {id: 'knowhow', label: '꿀팁 & 정보', show: activeTab !== 'news'}, // Renamed
  ...
  const showPointReward = ['praise', 'knowhow', 'matjib', 'our_news'].includes(writeCategory);
      ...
      <div className="bg-white w-full max-w-md rounded-[2.5rem] p-0 shadow-2xl max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="p-6">
            ...
            <div className="space-y-4 mb-8">
                ...
                {(writeCategory === 'knowhow' || writeCategory === 'news' || writeCategory === 'our_news') && (
...
// ... (Rest of the code like RankingTab, BottomNav, App component logic mostly remains the same, but I'll include the User Gift function in App) ...

// --- Main App Component (Updated with User Gift) ---
export default function App() {
  ...
  const [showUserGiftModal, setShowUserGiftModal] = useState(false); // NEW state
  ...
  // ... (Other useEffects and callbacks remain the same as previous optimal version) ...
  // Re-pasting the critical ones to ensure completeness
  ...
  const fetchFeeds = useCallback(async () => {
    if (!supabase) return; 
    try {
        const { data: posts } = await supabase.from('posts').select(`*, profiles:author_id (name, dept, team, role), comments (*, profiles:author_id (name, role))`).order('created_at', { ascending: false });
  ...
  const handleAddComment = async (e, postId, parentId = null) => {
      ...
      const tempComment = {
          ...
          profiles: { name: currentUser.name, role: currentUser.role }
      };
      setFeeds(prevFeeds => prevFeeds.map(feed => {
          if (feed.id === postId) {
              return { ...feed, comments: [...feed.comments, tempComment], totalComments: feed.totalComments + 1 };
          }
          return feed;
      }));
      e.target.reset(); 
  ...
  const handleDeleteComment = async (commentId) => {
      ...
      setFeeds(prevFeeds => prevFeeds.map(feed => {
          ...
          if (updatedComments.length !== feed.comments.length) {
              return { ...feed, comments: updatedComments, totalComments: updatedComments.length };
          }
          return feed;
      }));
      try { await supabase.from('comments').delete().eq('id', commentId); } 
      catch (err) { console.error('Delete failed:', err); fetchFeeds(); }
  };
  ...
  const handleDeletePost = async (postId) => {
    ...
    try {
        ...
        if (['praise', 'knowhow', 'matjib', 'our_news'].includes(postToDelete.type)) {
            ...
            else if (postToDelete.type === 'our_news') reasonText = '게시글 삭제(우리들 소식) - 회수';
  ...
  const handleLogin = async (e) => {
    ...
    try {
        ...
        if (error) {
            if (userCheck === null) alert('가입되지 않은 이메일 계정입니다.');
  ...
  const handlePostSubmit = async (e) => {
    ...
    const isRewardCategory = ['praise', 'knowhow', 'matjib', 'our_news'].includes(category);
  ...
  const handleMoodCheck = async (selectedMood) => {
    ...
    let message = "";
    let emoji = "";
    if (selectedMood === 'happy') { message = "오늘 기분 최고예요! 뭐든 할 준비 완료! 😄"; emoji = "✨"; } 
    else if (selectedMood === 'soso') { message = "괜찮아요! 오늘도 잘 해낼 거예요! 💪"; emoji = "🍀"; } 
    else if (selectedMood === 'sad') { message = "조금 지쳤지만 버틸 수 있어요.. 🐌"; emoji = "☕"; }
  ...
  // 일반 사용자 포인트 선물 처리 함수
  const handleUserGiftPoints = async (targetUserId, amount) => {
      if (!currentUser || !supabase) return;
      const giftAmount = parseInt(amount);
      if (giftAmount <= 0) return alert('유효한 포인트를 입력해주세요.');
      if (currentUser.points < giftAmount) return alert('보유 포인트가 부족합니다.');

      try {
          // 1. 받는 사람 정보 조회
          const { data: targetUser } = await supabase.from('profiles').select('points').eq('id', targetUserId).single();
          if (!targetUser) return alert('사용자를 찾을 수 없습니다.');
          ...
          // 4. 히스토리 기록 (내꺼)
          await supabase.from('point_history').insert({ user_id: currentUser.id, reason: '동료에게 선물 보냄', amount: giftAmount, type: 'use' });

          // 5. 히스토리 기록 (상대방)
          await supabase.from('point_history').insert({ user_id: targetUserId, reason: `${currentUser.name}님의 선물`, amount: giftAmount, type: 'earn' });

          setShowUserGiftModal(false);
          alert('포인트를 선물했습니다! 🎁');
          fetchUserData(currentUser.id); // 내 정보 갱신
          ...
          {!session ? (
            ...
            <>
              <Header 
                ...
                onOpenUserGift={() => setShowUserGiftModal(true)}
              ...
              {showUserGiftModal && <UserGiftModal onClose={() => setShowUserGiftModal(false)} onGift={handleUserGiftPoints} profiles={profiles} currentUser={currentUser} />}
